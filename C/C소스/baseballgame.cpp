...
void ComSel(int *arr)	// 컴퓨터가 3개의 숫자를 선택
{
	...
	do{					
		arr[1]=rand()%10;	//십의 자리 숫자 지정(백의 자리와 겹치지 않을 때 까지) 
	}while(arr[0]==arr[1]);
	do{					
		arr[2]=rand()&10;	//일의 자리 숫자 지정(백의 자리와 십의 자리 숫자가 겹치지 않을 때 까지) 
	}while((arr[0]==arr[2])||(arr[1]==arr[2])); 
}

int IsValid(int *arr) 	//user가 올바른 숫자입력했는지 검사  
{
	if(arr[0]==arr[1]||arr[0]==arr[2]||arr[1]==arr[2])	//유저가 선택한 숫자 3개가 겹친다면  
		return 0;	//0값을 반환 
	else	//겹치지 않는 다면  
		return 1;	//1값을 반환  
}
...
void UsrSel(int *arr)	//유저가 입력한 숫자 받는 함수  
{
	...
	while(1)
	{
		scanf("%1d %1d %1d", &arr[0], &arr[1], &arr[2]);	//하나씩만 입력받게 하기 위해 %1d로 함. 
